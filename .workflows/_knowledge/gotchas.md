...
□ Tour physics numbers: NEVER hardcode in TourDiagram.js act code — read from TOUR_PHYSICS (tourPhysics.js). The EHT 2017 max baseline is 11,406 km (IRAM↔SPT), θ=λ/B≈23.6 μas → displays "24" (.toFixed(0), matches the app). Not "25", not "20". Acts 1/4/8 must all read the same `P.str.thetaEht`.
...
□ TourDiagram shared utils (drawDerivationPanel/drawContactShadow/drawAxisTicks/drawScaleBar/drawForegroundAccent/drawConceptTag/drawHudFrame) are APPEND-ONLY helpers after drawEarth — reused across acts. Reset g.filter='none' after any blur inside them (drawForegroundAccent uses blur).
